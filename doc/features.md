...
- **Language Coverage & Data Integrity**:
  - **Japanese**: 828 patterns across JLPT N5 through N1 (`grammar-ja.ts`). All patterns are uniquely namespaced with their JLPT level prefix (`ja_{level}_{patternId}`, e.g. `ja_n5_A_0`, `ja_n1_A_0`), eliminating cross-level translation collisions where higher-level cards could overwrite beginner cards.
  - **Korean**: 744 patterns across Korean levels 1 through 6 (`grammar-ko.ts`). All 54 previously corrupt patterns (where pattern had collapsed to `"~"`) have their authentic grammatical affixes extracted and indexed.
  - **Chinese**: 691 patterns across HSK 1 through 6 (`grammar-zh.ts`).
  - **English**: 143 patterns across CEFR A1 through C2 (`grammar-en.ts`), with pedagogical and non-English translation artifacts thoroughly purged.
- **Multi-Strategy Detection Pipeline**:
  ...
  - **Strategy 2: Morphological Suffix Matching (JA & KO)**:
    ...
    - **Korean**: Checks `koEndingPatterns` (sorted longest-first) for attached particles (`이/가`, `은/는`, `을/를`, `에`, `에서`, `(으)로`, `(이)랑`) and conjugated verb endings (`-고`, `-아/어서`, `-면`, `-아/어요`, `-ㅂ니다/습니다`, `-네요`, `-지요`). Particle detachment protects 2-syllable nouns ending in `가` and supports `-(으)로` on `ㄹ`-batchim nouns.
...
2. **Ease Factor ($EF$)**:
   $$EF' = \text{clamp}(1.3, \; 3.0, \; \text{round}_2(EF + (0.1 - (5 - q) \times (0.08 + (5 - q) \times 0.02))))$$
   - Capped between $1.3$ and $3.0$ and rounded to 2 decimal places to prevent floating point drift and runaway review schedules. Interval strings format cleanly (e.g. 345+ days format as `1y` instead of `12mo`).
...
- **Responsive Video Screen Presentation**:
  ...
  - **Mobile Playlist Bar (`.mobile-playlist-card`) & YouTube-Style Bottom Sheet**:
    - Sits directly below the video player as a sleek, non-expanding compact bar (~48px) displaying playlist title, author, index/total, and an expand chevron.
    - **Zero Layout Shift (0% CLS)**: Tapping anywhere on the bar opens a modal `<app-bottom-sheet>` instead of expanding in-flow. The video player and subtitles beneath it remain stationary and completely undisturbed.
    - **Decluttered Mobile Header & Dedicated Sheet Toolbar**: Secondary actions (`Shuffle`, `Loop / Repeat`, `Share Playlist`) live inside the dedicated top header of the bottom sheet alongside the playlist title and creator metadata, keeping the main video watching screen focused and giving maximum vertical space to interactive subtitles. Inside the sheet, the user can reorder videos (cdkDrag if owner), switch videos, toggle loop/shuffle, share with copied feedback, or open individual video options. Selecting a video automatically closes the sheet and navigates to the video.
...
- **Service Worker, Server-Assisted Versioning & Changelog Architecture (`AppUpdateService`)**:
  ...
  - **Cloudflare Edge Headers & Content Security Policy (CSP)**: Configured in `public/_headers` with `no-cache, no-store, must-revalidate` for `ngsw.json` and `index.html`, and `immutable` for hashed JavaScript and CSS bundles. Enforces a strict Content Security Policy allowing YouTube IFrame player scripts (`https://s.ytimg.com`, `https://*.youtube.com`, `https://*.ytimg.com`), creator/user avatars (`https://*.ggpht.com`, `https://*.googleusercontent.com`), video thumbnails (`https://*.ytimg.com`, `https://i.ytimg.com`, `https://img.youtube.com`), payment QR codes (`https://img.vietqr.io`, `https://api.qrserver.com`), Supabase WebSockets (`wss://*.supabase.co`), and mirrors media/font origins in `connect-src` (`fonts.googleapis.com`, `fonts.gstatic.com`, `*.ytimg.com`, `*.googleusercontent.com`) to prevent Service Worker (`ngsw-worker.js`) fetch interception violations.
