...
- **Verified Database Transcript Video Recommendations (`VideoRecommendationService`)**:
  ...
  - **Dedicated Multi-Language Sub-Badges**: Subtitle languages are displayed as dedicated metadata sub-badges (e.g. 🇯🇵 `JA`, 🇯🇵 `JA` / 🇬🇧 `EN`, or 🇨🇳 `ZH +2`) in the card sub-row. Target learning languages are prioritized first, and BCP 47 codes (e.g. `zh-CN`, `en-US`) are normalized with standard circular SVG flags (`.circle-flag--xs`).
