...
- **Payload**:
  ...
  ```
   1. **R2 Multi-Language Cache Check, Pre-Baked Rich Tokens & AI Isolation**:
      - **Pre-Baked Rich Subtitles**: Transcripts saved to Cloudflare R2 (`transcripts/{videoId}/{lang}.json`) are pre-baked with full morphological tokens (`tokens: Token[]`), character-level ruby parts, Pinyin, and lemmas (`enrichSegmentsWithTokens`). When clients fetch `/api/transcript`, the returned `segments` already include tokens, allowing the frontend player to render Furigana/Pinyin with 0ms delay and skip separate `/api/tokenize-batch` network calls entirely.
      - **Self-Healing Legacy Upgrade**: When reading a legacy transcript from R2 that lacks tokens, the server automatically enriches the segments and writes the upgraded rich transcript back to R2 in the background via `context.waitUntil(saveTranscriptToR2(...))`.
  ...
  5. **AI Generation (Gladia V2 Pre-Recorded) - Asynchronous Webhook-Driven Pipeline**:
     ...
     - **Active AI Job Short-Circuit & Freshness Guard**: Checks D1 `ai_transcription_jobs` (`getActiveAiJob`) first. If an AI job is already processing, returns `{ status: 'processing', jobId }`. However, if `forceRefresh: true` is passed (e.g. user requested regeneration after a timeout or error), any existing queued/processing record is superseded and deleted (`deleteAiJob`), ensuring a completely fresh session and Gladia submission without reusing dead sessions.
  6. **Opaque Job Status Polling & Server-Side Self-Healing (`pollAiJobStatus`)**:
     ...
     - **R2 Storage Self-Healing**: If `jobId` is missing from D1 (due to retention expiration or dev server reboot), the endpoint checks Cloudflare R2 for `transcripts/{videoId}/{lang}.json`. If completed segments exist, it self-heals and immediately delivers `status: 'done'` with full cues.
...
- **`context.waitUntil()` Background Processing**:
  ...
  - On `transcription.success`: Robustly parses transcript payload via `extractGladiaSegments` (supporting Gladia v2 `item.sentence` semantic sentences, `item.text` utterances, and nested formats), normalizes language codes (including ISO 639-2/3 like `cmn`/`jpn`/`kor`), pre-bakes morphological tokens (`enrichSegmentsWithTokens`), commits permanently to Cloudflare R2 (`transcripts/{videoId}/{lang}.json`), updates D1 `video_languages`, and marks D1 `ai_transcription_jobs` as `'completed'`.
