...
This document outlines the frontend design principles, Angular 19 Signal state architecture, component hierarchy, repository patterns, and UI design system for **LinguaTube**.
...
└────────────────────────────────────────────────────────────────────────┘
 ...
 [ 1. Fine-Grained Reactivity ]       [ 2. Offline-First Data Layer ]
   ...
   • ChangeDetectionStrategy.OnPush      • LocalStorage + IndexedDB Persistence
 ...
 [ 3. Standalone Component Tree ]     [ 4. Cross-Platform Responsive UI ]
   ...
   • Deferred Loading (loadComponent)     • Touch Gestures & Media Session API
   • Isolated SCSS per component          • Full PWA Offline Asset Caching
```
...
LinguaTube uses Angular Signals as the single source of truth for all synchronous and reactive state.

### 2.1. Why Signals over NgRx or Manual BehaviorSubjects?
- **Glitch-Free Computed Derivations**: `computed()` values update lazily and deterministically without diamond-problem glitches.
...
- **`SidebarComponent`**: Collapsible main navigation supporting compact icon mode and expanded text mode. Displays current streak count and diamond credit badge.
- **`SettingsSheetComponent`**: Slide-over or modal sheet for adjusting learning languages, Furigana/Pinyin toggles, font size, playback speed, and theme.
- **`StreakDialogComponent`**: Modal displaying 7-day practice activity, streak freeze inventory, and milestone badges.
- **`AiCreditsDialogComponent`**: Details current Diamond credit count, regeneration countdown timer (20m intervals), and Gladia AI limits.
...
- **Phonetic Guidance**:
  - Japanese: Furigana annotations (`<ruby><rt>`) or Hepburn Romaji.
  - Chinese: Pinyin annotations above Hanzi.
...
- **`WordPopupComponent`**:
  ...
  - Displays headword, phonetic readings, parts of speech, English/native definitions, JLPT/HSK level badges, and source audio pronunciation.
...
- **`GrammarPopupComponent`**:
  ...
  - Shows formation rules, English & native explanations, JLPT/HSK level, and contextual example sentences.
...
- **`_layout.scss`**: Main grid, sidebar layouts, topbar header, safe area padding (`--bottom-nav-safe-area`).
