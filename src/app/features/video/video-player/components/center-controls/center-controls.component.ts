    ...
    <div class="center-controls" [class.has-playlist]="hasPlaylist()">
      ...
      @if (isReady() && !isEnded()) {
        <div class="center-button-group">
          ...
          @if (isBuffering()) {
            <button class="big-play-btn buffering" aria-label="Loading video" disabled
              (touchstart)="$event.stopPropagation()" (click)="$event.stopPropagation()">
              <app-icon name="loader" [size]="44" aria-hidden="true" />
            </button>
          } @else if (!isPlaying()) {
            <button class="big-play-btn" aria-label="Play video"
              (touchstart)="$event.stopPropagation()"
            ...
            <button class="big-play-btn fade-in" aria-label="Pause video"
              (touchstart)="$event.stopPropagation()"
              (touchend)="onPlayPauseTouch($event)"
              (click)="onPlayPauseClick($event)">
