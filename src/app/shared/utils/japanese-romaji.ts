...
function getNextRomaji(source: string, index: number): string {
...
function getConsonantForSokuon(nextRomaji: string): string {
  if (!nextRomaji) {
    return '';
  }
...
function getLastVowel(text: string): string {
  const match = text.match(/[aeiou](?!.*[aeiou])/);
  return match ? match[0] : '';
}

export function isJapaneseKanaText(text: string | undefined | null): boolean {
  return !!text && KANA_TEXT_REGEX.test(text);
}

export function toJapaneseRomaji(text: string | undefined | null): string {
  if (!text) {
    return '';
  }
...
export function getJapaneseRomaji(reading?: string | null, surface?: string | null): string | undefined {
